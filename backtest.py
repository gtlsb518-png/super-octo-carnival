...
사용법:
  ...
  python backtest.py --symbols BTCUSDT
  python backtest.py --ema 9,21               # EMA 조합 바꿔서 1회 실행
  python backtest.py --compare                # 🔥 4가지 설정 비교 (추천!)
                                              #   ① EMA 34/55 (현재 봇 설정)
                                              #   ② EMA 12/26
                                              #   ③ EMA 9/21
...
UT_ATR = 2
WARMUP = 60
LIQ_MOVE_PCT = 100.0 / LEVERAGE   # 격리마진 근사 강제청산 가격변동 %

# 비교 모드 프리셋
PRESETS = [
    {'name': 'EMA34-55(현재)', 'ema_fast': 34, 'ema_slow': 55, 'hybrid': False},
    {'name': 'EMA12-26',       'ema_fast': 12, 'ema_slow': 26, 'hybrid': False},
    {'name': 'EMA9-21',        'ema_fast': 9,  'ema_slow': 21, 'hybrid': False},
    {'name': '하이브리드',      'ema_fast': 34, 'ema_slow': 55, 'hybrid': True,
     'hybrid_fast': 9, 'hybrid_slow': 21, 'hybrid_roi_th': -5.0},
]
...
def run_backtest(df, cfg):
    ...
    def open_pos(side, i):
        """봉 i 신호 → 봉 i+1 시가 진입 (봇은 봉 마감 직후 시장가)"""
        entry = o[i + 1]
        tp = tp_pct_at(i)
    ...
    while i < len(df) - 1:
        ...
        if pos['side'] == 'LONG':
            ...
            roi_close = (c[j] - pos['entry']) / pos['entry'] * 100 * LEVERAGE
            ...
            elif cfg.get('hybrid') and roi_close <= roi_th and h_dead[j]:
                close_pos(pos, o[j + 1], j, '조기청산')
                pos = None
        else:  # SHORT
            liq_price = pos['entry'] * (1 + LIQ_MOVE_PCT / 100)
            roi_low = (pos['entry'] - h[j]) / pos['entry'] * 100 * LEVERAGE
            roi_close = (pos['entry'] - c[j]) / pos['entry'] * 100 * LEVERAGE
            pos['min_roi'] = min(pos['min_roi'], roi_low)
            if h[j] >= liq_price:
                close_pos(pos, liq_price, j, '강제청산')
                pos = None
                ...
                close_pos(pos, o[j + 1], j, '스위칭')
                pos = open_pos('LONG', j)
            elif cfg.get('hybrid') and roi_close <= roi_th and h_gold[j]:
                close_pos(pos, o[j + 1], j, '조기청산')
                pos = None
...
def summarize(trades, symbol, cfg_name, max_dd, days, verbose=True):
    if trades.empty:
        print(f"\n### {symbol} [{cfg_name}]: 거래 없음")
        return None
    total = len(trades)
    ...
    fees = trades['수수료'].sum()
    net = trades['순손익'].sum()
    tp_n = int((trades['유형'] == 'TP익절').sum())
    sw_n = int((trades['유형'] == '스위칭').sum())
    ec_n = int((trades['유형'] == '조기청산').sum())
    liq_n = int((trades['유형'] == '강제청산').sum())
    ...
    if verbose:
        print(f"\n{'=' * 66}")
        print(f"### {symbol} [{cfg_name}] — {days}일 (진입금 {AMOUNT:.0f} USDT × {LEVERAGE}x)")
        print(f"{'=' * 66}")
        print(f"  거래 횟수     : {total:,}회 (TP {tp_n} / 스위칭 {sw_n} / 조기청산 {ec_n} / 강제청산 {liq_n})")
        print(f"  승 / 패       : {wins} / {losses}  → 승률 {win_rate:.1f}%")
        print(f"  총 수익       : {gross:+,.2f} USDT | 총 수수료: -{fees:,.2f} USDT")
    ...
    return {
        ...
        '승률%': round(win_rate, 1), 'TP익절': tp_n, '스위칭': sw_n,
        '조기청산': ec_n, '강제청산': liq_n,
        '총수익': round(gross, 2), '수수료': round(fees, 2), '순손익': round(net, 2),
        '평균승': round(avg_win, 2), '평균패': round(avg_loss, 2),
        '최대단일손실': round(worst, 2), '최저ROI%': round(worst_roi, 1),
        '최대낙폭': round(max_dd, 2),
    }


def main():
    ap = argparse.ArgumentParser(description='봇 코드 기반 백테스터')
    ap.add_argument('--symbols', default='BTCUSDT,XRPUSDT,DOGEUSDT')
    ap.add_argument('--days', type=int, default=365)
    ap.add_argument('--ema', default='34,55', help='EMA 조합 (예: 9,21)')
    ap.add_argument('--compare', action='store_true',
                    help='EMA 34/55·12/26·9/21·하이브리드 4종 비교')
    args = ap.parse_args()
    ...
    if args.compare:
        ...
        ef, es = [int(x) for x in args.ema.split(',')]
        configs = [{'name': f'EMA{ef}-{es}', 'ema_fast': ef, 'ema_slow': es, 'hybrid': False}]

    print("=" * 66)
    print(f"🔬 백테스트: {', '.join(symbols)} | 최근 {args.days}일 | 5분봉")
    print(f"   UT({UT_SENS},{UT_ATR}) | TP 추세 {TP_TREND}% / 횡보 {TP_SIDEWAYS}%"
          f" | 수수료 {FEE_RATE * 100:.2f}%×2")
    print(f"   설정 {len(configs)}종: {', '.join(cfg['name'] for cfg in configs)}")
    print("=" * 66)
    ...
    for sym in symbols:
        ...
        for cfg in configs:
            ...
            row = summarize(trades, sym, cfg['name'], max_dd, args.days,
    ...
    if summaries:
        ...
        print(f"\n{'=' * 66}")
        print("📊 종합 비교표 (backtest_result.csv 저장됨)")
        print(f"{'=' * 66}")
        ...
        if args.compare:
            ...
            for name, grp in result.groupby('설정', sort=False):
                print(f"   {name:16s}: {grp['순손익'].sum():+10,.2f} USDT"
