...
GOLD_RULES = [
    ...
    ("1등이 던진 이 어마어마한 주문서 받아",
     ["1등이 던진", "이 어마어마한 주문서 받아"]),
    # 기업 이름과 수치(%)를 떼지 않기 ('삼화콘덴서' / '+26%' 방지)
    ("그리고 오늘 삼화콘덴서 +26%, 아모텍이 상한가로 미친 듯이 달리고 있어",
     ["그리고 오늘", "삼화콘덴서 +26%,", "아모텍이 상한가로", "미친 듯이 달리고 있어"]),
    ("삼화콘덴서 46%, 삼성전기 28% 잡아준 MLCC 맛집이 바로 이 공부방이야",
     ["삼화콘덴서 46%,", "삼성전기 28%", "잡아준 MLCC 맛집이", "바로 이 공부방이야"]),
    # 단 조사로 끝난 말 뒤에서는 지금처럼 끊어도 된다 ('주사를' / '5분 주사로')
    ("3시간 동안 팔에 꽂고 맞던 항암 주사를 5분 주사로 바꿔주는 기술이야.",
     ["3시간 동안 팔에", "꽂고 맞던 항암 주사를", "5분 주사로", "바꿔주는 기술이야."]),

    # ── C4081 기준 ────────────────────────────────────────────
    # 숫자와 단위를 떼지 않기 ('58만' / '원' 방지)
    ("걸어놓고 120일선 58만 원 문 앞이야",
     ["걸어놓고 120일선", "580,000원 문 앞이야"]),
    ("주주들은 이제 150만 원 박스 뚫는 거 팔짱 끼고 편하게 보면 돼",
     ["주주들은 이제 1,500,000원", "박스 뚫는 거", "팔짱 끼고 편하게 보면 돼"]),
    # 세는 말('석 달')을 떼지 않기
    ("6월에 242만 원 찍고 석 달 동안 150만 원 박스에 갇혀서",
     ["6월에 2,420,000원 찍고", "석 달 동안 1,500,000원", "박스에 갇혀서"]),
    # 천 단위 쉼표가 갈라진 숫자를 다시 붙이기 ('99,' + '800원에')
    ("영상에서 삼성전기를 99, 800원에 잡아줬고 구독자들은 30% 수익 진행중일거야",
     ["영상에서 삼성전기를", "99,800원에", "잡아줬고 구독자들은", "30% 수익 진행중일거야"]),
]
...
# 금액은 '10,000원' 꼴로 붙인다 (사용자 요청)
GOLD_MONEY = [
    ("지금 10만 원인데", "지금 100,000원인데"),
    ("1차 매물대 7만 원만", "1차 매물대 70,000원만"),
    ("99, 800원에 추천했고", "99,800원에 추천했고"),
    ("2만 2천 원", "22,000원"),
    ("10000원", "10,000원"),
    ("10,000 원짜리", "10,000원짜리"),
    ("6,500억 원", "6,500억원"),                       # 억·조는 붙이기만
    ("유가 오르면 원가가 아니라", "유가 오르면 원가가 아니라"),   # 금액 아님
    ("구독자들한테 줄 원픽은", "구독자들한테 줄 원픽은"),
    ("5일 원 다시", "5일 원 다시"),
]
...
def run_money():
    ok = 0
    for src, want in GOLD_MONEY:
        got = " ".join(w["word"] for w in S.format_money_tokens(S.merge_number_tokens(
            [{"word": w, "start": 0.0, "end": 0.0} for w in src.split()])))
        if got == want:
            ok += 1
        else:
            print(f"  X  {src!r} → {got!r} (기준 {want!r})")
    print(f"  금액 표기: {ok}/{len(GOLD_MONEY)} 통과")
    return ok == len(GOLD_MONEY)
...
if __name__ == "__main__":
    ...
    all_ok = a and b and c and d and e and f and g and h and m
    print(f"\n원고 기준 총 {n_lines}줄")
